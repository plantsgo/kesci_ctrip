...
del leak
feature_train=pd.read_csv("all_result_v29_24680_train_feature.csv")[["prob","orderid","basicroomid","predict_roomid"]]
feature_train.columns=["prob","orderid","basicroomid","roomid"]
feature_test=pd.read_csv("all_result_v29_24680_test_feature.csv")[["prob","orderid","basicroomid","predict_roomid"]]
feature_test.columns=["prob","orderid","basicroomid","roomid"]

feature_train_2=pd.read_csv("all_result_v31_24680_train_feature.csv")[["prob","orderid","basicroomid","predict_roomid"]]
feature_train_2.columns=["prob_2","orderid","basicroomid","roomid"]
feature_test_2=pd.read_csv("all_result_v31_24680_test_feature.csv")[["prob","orderid","basicroomid","predict_roomid"]]
feature_test_2.columns=["prob_2","orderid","basicroomid","roomid"]
...
#trick_basic_comment_ratio
trick_basic_comment_ratio=pd.read_csv("feature_basic_comment_ratio_train.csv").append(pd.read_csv("feature_basic_comment_ratio_test.csv"))
trick_basic_comment_ratio.columns=["basicroomid","orderdate","trick_basic_comment_ratio"]
trick_basic_comment_ratio["orderdate"]=trick_basic_comment_ratio["orderdate"]-1
...
for j in [1,3,5,7,9]:
    ...
    all=all.merge(feature_train_2,on=["orderid","basicroomid","roomid"],how="left")
    ...
    all = all.merge(trick_basic_comment_ratio, on=["basicroomid", "orderdate"], how="left")
    all["basic_comment_ratio_today"] = all["trick_basic_comment_ratio"] - all["basic_comment_ratio"]

    all = merge_max(all, ["orderid"], "basic_trick_today", "basic_trick_today_max")
    all = merge_max(all, ["orderid"], "room_trick_today", "room_trick_today_max")
    all = merge_max(all, ["orderid"], "basic_30days_realratio_today", "basic_30days_realratio_today_max")
    all = merge_max(all, ["orderid"], "room_30days_realratio_today", "room_30days_realratio_today_max")

    all["roomid_ori"] = all["roomid"]
    all["roomid"] = map(lambda x, y: int(str(x)[:-len(str(y))]), all["roomid"], all["rank"])

    #使用prob构造特征
    all=merge_max(all,["orderdate","basicroomid"],"prob","orderdate_basic_prob_max")
    all=merge_max(all,["orderdate","basicroomid","roomid"],"prob","orderdate_room_prob_max")
    all=merge_sum(all,["orderdate","basicroomid"],"prob","orderdate_basic_prob_sum")
    all=merge_sum(all,["orderdate","basicroomid","roomid"],"prob","orderdate_room_prob_sum")
    all=merge_mean(all,["orderdate","basicroomid"],"prob","orderdate_basic_prob_mean")
    all=merge_mean(all,["orderdate","basicroomid","roomid"],"prob","orderdate_room_prob_mean")
...
for j in range(10):
    ...
    all = all.merge(feature_test_2, on=["orderid", "basicroomid", "roomid"], how="left")
    #test中的basic_30days_ordnumratio数据异常，需要重新设置
    del all["basic_30days_ordnumratio"]
    all = all.merge(trick_basic_30d_fill, on=["hotelid", "basicroomid","orderdate"], how="left")
    #20170620添加特征
...
result.to_csv("all_result_v43_13579_test_feature.csv",index=None)
...
result.to_csv("sub_v43_13579.csv",index=None)
